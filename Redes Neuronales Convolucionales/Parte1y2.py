...
def mostrarEvolucion(hist, name):
...
def mostrarStats(score, hist, name, show = True):
...
def train(model, model_name, datagen, x_train, y_train,
    ...
    hist = model.fit_generator(datagen.flow(x_train,
                                            ...
                                            subset = 'training'),
                               epochs = EPOCHS,
                               steps_per_epoch = len(x_train) * (1 - 0.1) / 64,
                               verbose = 1,
                               validation_data = datagen.flow(x_train,
                                                              y_train,
                                                              batch_size = 64,
                                                              subset = 'validation'),
                               validation_steps = len(x_train) * 0.1 / 64,
...
def ex1(show = True):
    ...
    mostrarStats(score, hist, "basenet", show = show)
...
def ex2(show = True):
    ...
    # Mostramos estadísticas
    mostrarStats(score, hist, "improved_basenet", show = show)
